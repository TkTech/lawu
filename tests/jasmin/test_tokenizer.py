...
def test_quoted_string():
    tokens = list(tokenize(StringIO('"No Escapes"')))
    assert len(tokens) == 2
    ...
    tokens = list(tokenize(StringIO('"Escaped \\"Quote"')))
    assert len(tokens) == 2
    assert tokens[0].token_type == TokenType.QUOTED_STRING
...
def test_comment():
    tokens = list(tokenize(StringIO("; Test Comment")))
    assert len(tokens) == 2
...
def test_semicolon_in_generic():
    tokens = list(tokenize(StringIO("abc;123")))
    assert len(tokens) == 2
