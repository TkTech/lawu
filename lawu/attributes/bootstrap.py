...
BootstrapMethod = namedtuple("BootstrapMethod", ["method_ref", "bootstrap_args"])
...
class BootstrapMethodsAttribute(Attribute):
    ...
    def __init__(self, table, name_index=None):
        super(BootstrapMethodsAttribute, self).__init__(
            table,
            name_index or table.cf.constants.create_utf8("BootstrapMethods").index,
        )
        self.table = []

    def __repr__(self):
        return "<BootstrapMethods(table={self.table})>".format(self=self)
    ...
    def pack(self):
        ...
        for table_entry in self.table:
            out.write(
                pack(">HH", table_entry.method_ref, len(table_entry.bootstrap_args))
            )
            out.write(
                pack(
    ...
    def unpack(self, info):
        ...
        for _ in repeat(None, length):
            self.table.append(
                BootstrapMethod(info.u2(), info.unpack(">{0}H".format(info.u2())))
            )
