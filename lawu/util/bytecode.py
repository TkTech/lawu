...
_Instruction = namedtuple("Instruction", ["mnemonic", "opcode", "operands", "pos"])
...
class Instruction(_Instruction):
    ...
    def create(cls, mnemonic_or_op, operands=None):
        ...
        return cls(op["mnemonic"], op["op"], operands or [], 0)
...
def read_instruction(fio, start_pos):
    ...
    if operands:
        for fmt, type_ in operands:
            final_operands.append(
                Operand(type_, fmt.value.unpack(fio.read(fmt.value.size))[0])
            )
    # Special case for lookupswitch.
    elif op == 0xAB:
    ...
    # Special case for the wide prefix
        ...
        final_operands.append(
            Operand(OperandTypes.LOCAL_INDEX, unpack(">H", fio.read(2))[0])
        )
        # Further special case for iinc.
        if real_op == 0x84:
            final_operands.append(
                Operand(OperandTypes.LITERAL, unpack(">H", fio.read(2))[0])
            )
...
def load_bytecode_definitions(*, path=None) -> dict:
    ...
    if path is not None:
        ...
        try:
            j = json.loads(pkgutil.get_data("lawu.util", "bytecode.json"))
    ...
    for definition in j.values():
        ...
        if operands:
            definition["operands"] = [
                [getattr(OperandFmts, oo[0]), OperandTypes[oo[1]]] for oo in operands
            ]
